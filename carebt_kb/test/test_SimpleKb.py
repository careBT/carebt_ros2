...
class TestSimpleKb():
    ...
    def test_create_from_file(self):
        kb = SimpleKb('src/carebt_ros2/carebt_kb/test/data/memory.json')
        assert len(kb._SimpleKb__kb_dict) == 4

    def test_get_uuids(self):
        kb = SimpleKb('src/carebt_ros2/carebt_kb/test/data/memory.json')

        result = kb._SimpleKb__get_uuids({'is-a': 'Person', 'name': 'Bob'})
        assert len(result) == 1
        assert result[0] == '7a84f3c5-9c60-491c-8dde-38e2572fc970'

        result = kb._SimpleKb__get_uuids({'is-a': 'Person', 'name': 'Alice'})
        assert len(result) == 2

        result = kb._SimpleKb__get_uuids({'is-a': 'Person', 'name': 'XXX'})
        assert len(result) == 0

        result = kb._SimpleKb__get_uuids({'is-a': 'Person'})
        print(result)
        assert len(result) == 4
    ...
    def test_create(self):
        ...
        assert len(kb._SimpleKb__kb_dict) == 5
