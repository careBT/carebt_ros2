...
from carebt_nav2.utility_nodes import WaitForUserInput
...
class FollowWaypointsSequence(RosActionServerSequenceNode):
    ...
    def execute_callback(self, goal_handle):
        ...
        waypoint_action =\
            self.__ros_node.get_parameter('waypoint_action').get_parameter_value().string_value
        for _ in self._poses:
            self.append_child(ApproachPose, '?pose => ?feedback')
            if(waypoint_action == 'WaitForUserInput'):
                print('add WaitForUserInput')
                self.append_child(WaitForUserInput)
            elif(waypoint_action == 'WaitAction'):
                print('add WaitAction')
                self.append_child(WaitAction, '2500')
